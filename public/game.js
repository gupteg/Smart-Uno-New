window.addEventListener('DOMContentLoaded', () => {
    ...
    // BRANCH C: Robo config modal elements
    ...
    configureAIBtn.addEventListener('click', () => {
        ...
        roboConfigModal.style.display = 'flex';
    });
    ...
    roboConfigConfirmBtn.addEventListener('click', () => {
        ...
        roboConfigModal.style.display = 'none';
    });
    ...
    // BRANCH C: Show robo config modal when host successfully claims host role
    ...
    function renderRoboConfigTable(count) {
        ...
        roboConfigTable.style.display = 'block';
        for (let i = 0; i < count; i++) {
    ...
    function renderLobby(currentLobbyPlayers) {
        ...
        currentLobbyPlayers.forEach(player => {
            ...
            // BRANCH C: AI badge for robo players in lobby
            if (player.isRobo) {
                const aiBadge = document.createElement('span');
                aiBadge.className = 'ai-badge';
                aiBadge.textContent = `🤖 AI · ${player.difficulty || 'Normal'}`;
                playerInfoDiv.appendChild(aiBadge);
            }
    ...
    makeDraggable(document.getElementById('robo-config-modal')); /* BRANCH C */
